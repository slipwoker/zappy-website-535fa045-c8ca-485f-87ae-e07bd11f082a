    ...
    fields.forEach(function (field) {
      ...
      if (field.hasAttribute('required') && value === '') {
        ...
        if (type === 'email' && !isValidEmail(value)) {
          ...
          if (value.length < min) {
            errorMessage = getFieldLabel(field) + ' must be at least ' + min + ' characters.';
          }
        } else if (field.hasAttribute('maxlength')) {
          const max = parseInt(field.getAttribute('maxlength'), 10);
          if (value.length > max) {
            errorMessage = getFieldLabel(field) + ' must be no more than ' + max + ' characters.';
          }
        }
      }
      ...
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          ...
          if (node.matches && node.matches(OBSERVED_SELECTOR)) {
            ...
            const children = node.querySelectorAll(OBSERVED_SELECTOR);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מוניות רחובות והסביבה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '535fa045-c8ca-485f-87ae-e07bd11f082a',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '535fa045-c8ca-485f-87ae-e07bd11f082a';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '535fa045-c8ca-485f-87ae-e07bd11f082a';
